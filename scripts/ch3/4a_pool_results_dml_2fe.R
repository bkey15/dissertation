...
treat_names <- names(imp_dml_fits_2fe[[1]])
m <- 1:length(imp_dml_fits_2fe[[1]][[1]])
  ...
  for(year in start_yrs){
    ...
    for(name in treat_names){
      ...
      pool_res <- prepool_tbl |> 
        pool.table()
      imp_dml_pool_2fe[[as.character(stat)]][[as.character(year)]][[as.character(name)]] <- pool_res
    }
  }
}
...
# save ----
imp_dml_pool_2fe |> 
  save(file = here("data/ch2/results/fits/dml_lasso/dml_final/imp_dml_pool_2fe.rda"))
