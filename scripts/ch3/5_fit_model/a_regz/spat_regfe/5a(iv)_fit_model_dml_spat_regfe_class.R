...
# run prep script ----
source(here("scripts/ch3/4_prep_model/a_regz/spat_regfe/4a(iv)_prep_model_dml_spat_regfe_class.R"))
...
# fit models ----
## get initial specs ----
imp_dml_fits_spat_regfe_class <- list()
      ...
      for(treat in treat_names){
        ...
        for(i in m){
          ...
          if(str_detect(treat, "inforce_X1")){
            ...
            fit <- spec$fit()
            toc(log = TRUE)
            imp_dml_fits_spat_regfe_class[[as.character(stat)]][[as.character(year)]][[as.character(lag)]][[as.character(treat)]][[as.character(i)]] <- fit
          }
          else{
            spec <- dml$DoubleMLPLR(
              obj_dml_data = dml_dat_i,
...
tictoc_imp_dml_fits_spat_regfe_class <- tic.log()

# convert to python object ----
imp_dml_fits_spat_regfe_class <- imp_dml_fits_spat_regfe_class |> 
  r_to_py()

# save fits ----
## note: compressing w/ joblib & .gz ext.
imp_dml_fits_spat_regfe_class |> 
  jlb$dump(filename = here("data/ch3/results/fits/dml_regularize/full_dat/imp_dml_fits_spat_regfe_class.gz"), compress = int(3))
tictoc_imp_dml_fits_spat_regfe_class |> 
  save(file = here("data/ch3/results/fits/dml_regularize/runtimes/tictoc_imp_dml_fits_spat_regfe_class.rda"))
