...
# run prep script ----
source(here("scripts/ch3/4_prep_model/a_regz/spat_regfe/4a(iii)_prep_model_dml_spat_regfe_regr.R"))
...
# fit models ----
## get initial specs ----
imp_dml_fits_spat_regfe_regr <- list()
    ...
    for(lag in lag_names){
      ...
      for(treat in treat_names){
        ...
        for(i in m){
          ...
          toc(log = TRUE)
          imp_dml_fits_spat_regfe_regr[[as.character(stat)]][[as.character(year)]][[as.character(lag)]][[as.character(treat)]][[as.character(i)]] <- fit
        }
      }
    }
  }
...
tictoc_imp_dml_fits_spat_regfe_regr <- tic.log()

# save fits ----
imp_dml_fits_spat_regfe_regr |> 
  save(file = here("data/ch3/results/fits/dml_regularize/full_dat/imp_dml_fits_spat_regfe_regr.rda"))
tictoc_imp_dml_fits_spat_regfe_regr |> 
  save(file = here("data/ch3/results/fits/dml_regularize/runtimes/tictoc_imp_dml_fits_spat_regfe_regr.rda"))
