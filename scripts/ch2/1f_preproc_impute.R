...
test <- c("n_bits", "partner_mean")

bits_1962 |> select(contains(test))
...
imp_vals <- bits_1962 |> 
  select(-1) |> 
  mutate(
    across(contains(no_imp_vars), ~ FALSE),
    across(
      !contains(no_imp_vars),
      ~ if_else(
        is.na(.x),
        TRUE,
        FALSE
...
### save ----
imp_base |> 
  save(file = here("data/ch2/results/imputations/imp_base.rda"))
