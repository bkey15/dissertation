...
# load data ----
load(here("data/ch2/results/fits/dml_lasso/dml_initial/imp_dml_dats_spat_regfe_gen.rda"))
...
start_yrs <- names(imp_dml_dats_spat_regfe_gen[[1]])
m <- 1:length(imp_dml_dats_spat_regfe_gen[[1]][[1]][[1]])
  ...
  for(year in start_yrs){
    ...
    for(name in treat_names){
      ...
      for(i in m){
        ...
        fit <- spec$fit()
        imp_dml_fits_spat_regfe_gen[[as.character(stat)]][[as.character(year)]][[as.character(name)]][[as.character(i)]] <- fit
      }
    }
  }
}
...
imp_dml_fits_spat_regfe_gen |> 
  save(file = here("data/ch2/results/fits/dml_lasso/dml_final/imp_dml_fits_spat_regfe_gen.rda"))
