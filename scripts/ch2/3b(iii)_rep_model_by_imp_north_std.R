# Replicate Bodea-Ye (2018) w/ two-way fixed effects & multiple imputation (`mice`). Fit models w/ imputed vals (& lagged covars). Limited to developed ("Global north") countries.

# load packages ----
library(tidyverse)
library(here)
library(mice)

# load data ----
load(here("data/ch2/results/imputations/imp_1962_l1.rda"))
load(here("data/ch2/results/imputations/imp_1981_l1.rda"))
load(here("data/ch2/results/imputations/imp_1990_l1.rda"))

# filter to north ----
imp_1962_l1_north <- imp_1962_l1 |> 
  mice::complete(
    action = "long",
    include = TRUE
    ) |> 
  filter(glb_s == 0)

## get treat names
### general
treat_names_gen <- imp_1962_l1_north |> 
  select(n_bits, starts_with("partner_")) |> 
  names()

### partner type (nn & ns)
treat_names_nn <- imp_1962_l1_north |> 
  select(starts_with("nn_")) |> 
  names()

treat_names_ns <- imp_1962_l1_north |> 
  select(starts_with("ns_")) |> 
  names()

## get interaction names
### general
interact_names_gen <- imp_1962_l1_north |> 
  select(
    starts_with("e_polity2_x_") & !contains(
      c("ss", "ns", "nn")
      )
    ) |> 
  names()

### partner type (nn & ns)
interact_names_nn <- imp_1962_l1_north |> 
  select(starts_with("e_polity2_x_nn")) |> 
  names()

interact_names_ns <- imp_1962_l1_north |> 
  select(starts_with("e_polity2_x_ns")) |> 
  names()

## as.mids()
imp_1962_l1_north <- imp_1962_l1_north |> 
  as.mids()

## continue filter
imp_1981_l1_north <- imp_1981_l1 |> 
  mice::complete(
    action = "long",
    include = TRUE
    ) |> 
  filter(glb_s == 0) |> 
  as.mids()

imp_1990_l1_north <- imp_1990_l1 |> 
  mice::complete(
    action = "long",
    include = TRUE
    ) |> 
  filter(glb_s == 0) |> 
  as.mids()

# no interaction terms ----
## start 1962 ----
start_1962 <- list()

### general ----
### i.e., no consideration for ns, nn, nn bits
for(name in treat_names_gen){
  formula <- paste0(
    "hr_score ~ ",
    name,
    " + e_polity2 + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1962_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1962[[as.character(name)]] <- sum
}

### BITs by partner-type (nn & ns) ----
for(i in seq_along(treat_names_nn)){
  j <- treat_names_nn[[i]]
  k <- treat_names_ns[[i]]
  
  formula <- paste0(
    "hr_score ~ ",
    j,
    " + ",
    k,
    " + e_polity2 + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1962_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1962[[paste(as.character(j), as.character(k), sep = "_AND_")]] <- sum
}

## start 1981 ----
start_1981 <- list()

### general ----
### i.e., no consideration for ns, nn, nn bits
for(name in treat_names_gen){
  formula <- paste0(
    "hr_score ~ ",
    name,
    " + e_polity2 + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1981_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1981[[as.character(name)]] <- sum
}

### BITs by partner-type (nn & ns) ----
for(i in seq_along(treat_names_nn)){
  j <- treat_names_nn[[i]]
  k <- treat_names_ns[[i]]
  
  formula <- paste0(
    "hr_score ~ ",
    j,
    " + ",
    k,
    " + e_polity2 + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1981_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1981[[paste(as.character(j), as.character(k), sep = "_AND_")]] <- sum
}

## start 1990 ----
start_1990 <- list()

### general ----
### i.e., no consideration for ns, nn, nn bits
for(name in treat_names_gen){
  formula <- paste0(
    "hr_score ~ ",
    name,
    " + e_polity2 + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1990_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1990[[as.character(name)]] <- sum
}

### BITs by partner-type (nn & ns) ----
for(i in seq_along(treat_names_nn)){
  j <- treat_names_nn[[i]]
  k <- treat_names_ns[[i]]
  
  formula <- paste0(
    "hr_score ~ ",
    j,
    " + ",
    k,
    " + e_polity2 + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1990_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1990[[paste(as.character(j), as.character(k), sep = "_AND_")]] <- sum
}

## combine ----
no_interactions <- list(
  start_1962 = start_1962,
  start_1981 = start_1981,
  start_1990 = start_1990
  )

# has interaction terms ----
## start 1962 ----
start_1962 <- list()

### general ----
### i.e., no consideration for ns, nn, nn bits
for(i in seq_along(treat_names_gen)){
  j <- treat_names_gen[[i]]
  k <- interact_names_gen[[i]]
  
  formula <- paste0(
    "hr_score ~ ",
    j,
    " + e_polity2 + ",
    k,
    " + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1962_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1962[[as.character(j)]] <- sum
}

### BITs by partner-type (nn & ns) ----
for(i in seq_along(treat_names_nn)){
  j <- treat_names_nn[[i]]
  k <- treat_names_ns[[i]]
  l <- interact_names_nn[[i]]
  n <- interact_names_ns[[i]]
  
  formula <- paste0(
    "hr_score ~ ",
    j,
    " + ",
    k,
    " + e_polity2 + ",
    l,
    " + ",
    n,
    " + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1962_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1962[[paste(as.character(j), as.character(k), sep = "_AND_")]] <- sum
}

## start 1981 ----
start_1981 <- list()

### general ----
### i.e., no consideration for ns, nn, nn bits
for(i in seq_along(treat_names_gen)){
  j <- treat_names_gen[[i]]
  k <- interact_names_gen[[i]]
  
  formula <- paste0(
    "hr_score ~ ",
    j,
    " + e_polity2 + ",
    k,
    " + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1981_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1981[[as.character(j)]] <- sum
}

### BITs by partner-type (nn & ns) ----
for(i in seq_along(treat_names_nn)){
  j <- treat_names_nn[[i]]
  k <- treat_names_ns[[i]]
  l <- interact_names_nn[[i]]
  n <- interact_names_ns[[i]]
  
  formula <- paste0(
    "hr_score ~ ",
    j,
    " + ",
    k,
    " + e_polity2 + ",
    l,
    " + ",
    n,
    " + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1981_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1981[[paste(as.character(j), as.character(k), sep = "_AND_")]] <- sum
}

## start 1990 ----
start_1990 <- list()

### general ----
### i.e., no consideration for ns, nn, nn bits
for(i in seq_along(treat_names_gen)){
  j <- treat_names_gen[[i]]
  k <- interact_names_gen[[i]]
  
  formula <- paste0(
    "hr_score ~ ",
    j,
    " + e_polity2 + ",
    k,
    " + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1990_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1990[[as.character(j)]] <- sum
}

### BITs by partner-type (nn & ns) ----
for(i in seq_along(treat_names_nn)){
  j <- treat_names_nn[[i]]
  k <- treat_names_ns[[i]]
  l <- interact_names_nn[[i]]
  n <- interact_names_ns[[i]]
  
  formula <- paste0(
    "hr_score ~ ",
    j,
    " + ",
    k,
    " + e_polity2 + ",
    l,
    " + ",
    n,
    " + wdi_fdiin + wdi_trade + p_durable + gdppc_log10 + wdi_popden + v2cademmob + factor(cow) + factor(year)"
  )
  
  fit <- with(imp_1990_l1_north, lm(as.formula(formula)))
  
  pool <- pool(fit)
  sum <- summary(pool)
  
  start_1990[[paste(as.character(j), as.character(k), sep = "_AND_")]] <- sum
}

## combine ----
has_interactions <- list(
  start_1962 = start_1962,
  start_1981 = start_1981,
  start_1990 = start_1990
  )

# all combine ----
imp_by_rep_res_north_std <- list(
  no_interactions = no_interactions,
  has_interactions = has_interactions
  )


# save ----
imp_by_rep_res_north_std |> 
  save(file = here("data/ch2/results/fits/by_rep/mice/imp_by_rep_res_north_std.rda"))
