...
# load data ----
load(here("data/ch2/results/fits/dml_lasso/dml_initial/imp_dml_dats_2fe_north.rda"))
...
start_yrs <- names(imp_dml_dats_2fe_north[[1]])
treat_names <- names(imp_dml_dats_2fe_north[[1]][[1]])
m <- 1:5
  ...
  for(year in start_yrs){
    ...
    for(name in treat_names){
      ...
      for(i in m){
        ...
        imp_dml_fits_2fe_north[[as.character(stat)]][[as.character(year)]][[as.character(name)]][[as.character(i)]] <- fit
    }
    }
  }
  }
...
imp_dml_fits_2fe_north |> 
  save(file = here("data/ch2/results/fits/dml_lasso/dml_final/imp_dml_fits_2fe_north.rda"))
