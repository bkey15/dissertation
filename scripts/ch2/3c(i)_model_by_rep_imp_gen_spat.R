...
for(name in treat_names){
  ...
  fit <- with(imp_1962_sp_l1, lm(as.formula(formula)))
...
for(name in treat_names){
  ...
  fit <- with(imp_1981_sp_l1, lm(as.formula(formula)))
...
for(name in treat_names){
  ...
  fit <- with(imp_1990_sp_l1, lm(as.formula(formula)))
...
# all combine ----
imp_by_rep_res_gen_sp <- list(
  no_interactions = no_interactions,
  has_interactions = has_interactions
  )

# save ----
imp_by_rep_res_gen_sp |> 
  save(file = here("data/ch2/results/fits/by_rep/mice/imp_by_rep_res_gen_sp.rda"))
