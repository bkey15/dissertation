...
### get initial specs ----
m <- 1:imp_2_sp_l1$m
imp_2_dml_north_spat_dats <- list()
...
for(i in m){
  ...
  for(j in seq_along(treat_names_cpr)){
    k <- treat_names_cpr[[j]]
    l <- treat_names_esr[[j]]
    imp_2_dml_north_spat_dats[[paste(as.character(k), as.character(l), sep = "_AND_")]][[as.character(i)]] <- df |>
...
for(i in m){
  ...
  for(j in seq_along(treat_names_lech)){
    ...
    imp_2_dml_north_spat_dats[[as.character(k)]][[as.character(i)]] <- df |>
      double_ml_data_from_data_frame(
        x_cols = covar_names,
        d_cols = k,
        y_col = "hr_score"
...
for(i in m){
  ...
  for(j in seq_along(treat_names_nn_cpr)){
    ...
    imp_2_dml_north_spat_dats[[paste(as.character(k), as.character(l), as.character(n), as.character(o), sep = "_AND_")]][[as.character(i)]] <- df |> 
      double_ml_data_from_data_frame(
        x_cols = covar_names,
        d_cols = c(k, l, n, o),
        y_col = "hr_score"
...
### get initial specs ----
m <- 1:imp_3_sp_l1$m
imp_3_dml_north_spat_dats <- list()
covar_names <- model.matrix(
  ~ . - 1, data = imp_3_dfs[[1]]
  ) |> 
  as_tibble() |> 
...
for(i in m){
  ...
  for(j in seq_along(treat_names_cpr)){
    k <- treat_names_cpr[[j]]
    l <- treat_names_esr[[j]]
    imp_3_dml_north_spat_dats[[paste(as.character(k), as.character(l), sep = "_AND_")]][[as.character(i)]] <- df |>
      double_ml_data_from_data_frame(
        x_cols = covar_names,
        d_cols = c(k, l),
        y_col = "hr_score"
...
for(i in m){
  ...
  for(j in seq_along(treat_names_lech)){
    k <- treat_names_lech[[j]]
    imp_3_dml_north_spat_dats[[as.character(k)]][[as.character(i)]] <- df |>
      double_ml_data_from_data_frame(
        x_cols = covar_names,
        d_cols = k,
        y_col = "hr_score"
...
for(i in m){
  ...
  for(j in seq_along(treat_names_nn_cpr)){
    ...
    n <- treat_names_nn_esr[[j]]
    o <- treat_names_ns_esr[[j]]
    imp_3_dml_north_spat_dats[[paste(as.character(k), as.character(l), as.character(n), as.character(o), sep = "_AND_")]][[as.character(i)]] <- df |> 
      double_ml_data_from_data_frame(
        x_cols = covar_names,
        d_cols = c(k, l, n, o),
        y_col = "hr_score"
...
imp_2_dml_north_spat_dats |> 
  save(file = here("data/ch1/results/fits/dml_lasso/dml_initial/imp_2_dml_north_spat_dats.rda"))
imp_3_dml_north_spat_dats |> 
  save(file = here("data/ch1/results/fits/dml_lasso/dml_initial/imp_3_dml_north_spat_dats.rda"))
