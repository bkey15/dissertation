...
depth_names_gen_orig <- imp_t_dfs[[1]][[1]][[1]] |> 
  select(
    starts_with("depth"),
    -ends_with("sp_lag")
    ) |> 
...
depth_names_gen_sp_lag <- imp_t_dfs[[1]][[1]][[1]] |> 
  select(
    starts_with("depth") & ends_with("sp_lag")
    ) |> 
  names()

enforce_names_gen_orig <- imp_t_dfs[[1]][[1]][[1]] |> 
  select(
    starts_with("enforce"),
    -ends_with("sp_lag")
    ) |> 
...
enforce_names_gen_sp_lag <- imp_t_dfs[[1]][[1]][[1]] |> 
  select(
    starts_with("enforce") & ends_with("sp_lag")
    ) |> 
  names()

lech_names_gen_sp_lag <- imp_t_dfs[[1]][[1]][[1]] |> 
  select(
...
# all combine ----
imp_dml_dats_spat_cfe_gen <- list(
  no_interactions = no_interactions,
  has_interactions = has_interactions
  )

# clear glb env ----
rm(list = setdiff(ls(), "imp_dml_dats_spat_cfe_gen"))
gc()
