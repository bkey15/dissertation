...
### partner-type (nn & ns) ----
treat_names_nn <- imp_sp_t_dfs[[1]][[1]][[1]] |> 
  select(
    starts_with("nn_lech"),
    -ends_with("sp_lag")
    ) |> 
...
### partner-type (nn & ns) ----
interact_names_nn <- imp_sp_t_dfs[[1]][[1]][[1]] |> 
...
lech_names_nn_sp_lag <- imp_sp_t_dfs[[1]][[1]][[1]] |> 
  select(
    starts_with("nn_lech") & ends_with("sp_lag")
    ) |> 
  names()
...
# all combine ----
imp_dml_dats_spat_cfe_north <- list(
  no_interactions = no_interactions,
  has_interactions = has_interactions
  )

# clear glb env ----
rm(list = setdiff(ls(), "imp_dml_dats_spat_cfe_north"))
