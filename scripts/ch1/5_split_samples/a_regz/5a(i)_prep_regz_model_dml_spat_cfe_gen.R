...
source(here("scripts/ch1/4_prep_model/a_regz/spat_cfe/4a(iv)_prep_regz_model_dml_spat_cfe_gen.R"))
...
## note: imp_dml_dats_spat_cfe_gen has already been filtered such that only countries with polygons appear (see preproc code), so no need to filter further at this stage.
## note: economizing code, noting that row ids only differ by lag
interact_stat <- names(imp_dml_dats_spat_cfe_gen)
start_yrs <- names(imp_dml_dats_spat_cfe_gen[[1]])
lag_names <- names(imp_dml_dats_spat_cfe_gen[[1]][[1]])[1:4]
treat_names_reg <- names(imp_dml_dats_spat_cfe_gen[[1]][[1]][[1]])
treat_names_interact <- names(imp_dml_dats_spat_cfe_gen[[2]][[1]][[1]])
m <- 1:length(imp_dml_dats_spat_cfe_gen[[1]][[1]][[1]][[1]])
cow_yr_uniques <- list()
row_ids <- list()

for(year in start_yrs){
  for(lag in lag_names){
    dt <- imp_dml_dats_spat_cfe_gen[[1]][[year]][[lag]][[1]][[1]]$data[, .(cow, year)]
...
# }
# 
# overlap_check <- bind_rows(overlap_check)
...
## finalize splits ----
splits_imp_dml_dats_spat_cfe_gen <- list()
train_list <- list()
test_list <- list()
        ...
        for(i in m){
          ...
          for(rep in rep_num){
            ...
            for(fold in seq_along(fold_names)){
              ...
              splits_imp_dml_dats_spat_cfe_gen[[as.character(stat)]][[as.character(year)]][[as.character(lag)]][[as.character(treat)]][[as.character(i)]][[rep]] <- list(train_ids = train_list, test_ids = test_list)
            }
          }
        }
      }
...
rm(list = setdiff(ls(), "splits_imp_dml_dats_spat_cfe_gen"))
gc()

# save splits ----
splits_imp_dml_dats_spat_cfe_gen |> 
  save(file = here("data/ch1/results/splits/spat_cfe/splits_imp_dml_dats_spat_cfe_gen.rda"))
